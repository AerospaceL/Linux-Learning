    ...
    struct sockaddr *sa, int socklen, void *user_data)
{
	...
	bufferevent_write(bev, MESSAGE, strlen(MESSAGE)); // 给cfd发送消息
}

static void
conn_writecb(struct bufferevent *bev, void *user_data)
{
	struct evbuffer *output = bufferevent_get_output(bev); // 获取缓冲区的类型
...
signal_cb(evutil_socket_t sig, short events, void *user_data)
{
	...
	event_base_loopexit(base, &delay); // 2s后退出循环监听
}
